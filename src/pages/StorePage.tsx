...
export const StorePage = () => {
  ...
  const generateHupijiaoUrl = (method: string, amount: number, orderId: string) => {
    const appId = (method === 'wechat' ? config.hupiWechatAppId : config.hupiAlipayAppId) || '';
    const secret = (method === 'wechat' ? config.hupiWechatSecret : config.hupiAlipaySecret) || '';
    ...
    const sign = md5(signStr.slice(0, -1) + secret.trim());
    
    const queryString = new URLSearchParams({...params as any, hash: sign}).toString();
    const finalUrl = `${config.hupiGateway}?${queryString}`;
    setHupiPayUrl(finalUrl);
  };
